...
addpath([pwd,'/Sensory_Cross_Training_Alpha_Results/'])

 
Nsub = length(sublist);

Nblock = 9; % cross-validation
Nitr = 10; % iteration
...
for sub = 1:Nsub
    ...
    fileLocation = [pwd, '/Sensory_Cross_Training_Alpha_Results/'];
    
        readThis =strcat(fileLocation,'CA_Face_House_Sensory_Alpha_Decoding_',num2str(sublist(sub)),'.mat');
...
%% note: simulation takes very long time. 
 load(['Permutation_CA_FvH_Main_Cross_Sensory_Alpha_onetailed_ERP.mat'])
...
box off 
title('Alpha Cross Sensory Decoding') 

hold off
   saveas(figure(1),'Main_Alpha_Cross_Sensory','svg')
