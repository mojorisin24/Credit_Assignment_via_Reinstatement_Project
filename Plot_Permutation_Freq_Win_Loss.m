...
addpath([pwd,'/CA_Main_Alpha_Win_Loss_Results/'])

 
Nsub = length(sublist);

Nblock = 6; % cross-validation
Nitr = 100; % iteration
...
for sub = 1:Nsub
    ...
    fileLocation = [pwd, '/CA_Main_Alpha_Win_Loss_Results/'];
    
    if cond == 1  
        readThis =strcat(fileLocation,'CA_FvH_Alpha_Wins_Decoding_',num2str(sublist(sub)),'.mat');
    elseif cond == 2 
        readThis =strcat(fileLocation,'CA_FvH_Alpha_Losses_Decoding_',num2str(sublist(sub)),'.mat');
      end 
...
%% do cluster mass analyses
 releventTime = 26:100; % from 220 ms - 1496 ms
...
%% note: simulation takes very long time. 
    if cond == 1 
        load(['Permutation_CA_FvH_Alpha_Wins_Decoding_onetailed_ERP.mat'])
    elseif cond == 2 
        load(['Permutation_CA_FvH_Alpha_Losses_Decoding_onetailed_ERP.mat'])
    end 
...
box off 
    if cond == 1 
        title('Alpha Win Decoding') 
    elseif cond == 2 
        title('Alpha Loss Decoding')
    end 

hold off
    if cond == 1 
        saveas(figure(1),'Alpha_Win','svg')
    elseif cond == 2 
        saveas(figure(1),'Alpha_Loss','svg')
    end 
    close all; 
end  