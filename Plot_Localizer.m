...
addpath([pwd,'/CA_Localizer_Results/'])
...
for sub = 1:Nsub
    ...
    fileLocation = [pwd, '/CA_Localizer_Results/'];
    
        readThis =strcat(fileLocation,'CA_Localizer_Decoding',num2str(sublist(sub)),'.mat');
...
%% note: simulation takes very long time. 
        load(['Permutation_CA_Localizer_Decoding_onetailed_ERP.mat'])
...
box off 
        title('Localizer Decoding') 

hold off
   saveas(figure(1),'Localizer','svg')
